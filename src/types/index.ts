...
export interface UserContextType {
    ...
    logout: () => void;
    updateUser: (updatedData: Partial<User>) =>{t('common.promise')}<void>;
    startTrial: (featureKey: string) =>{t('common.promise')}<void>;
    hasActiveTrial: (featureKey: string) => boolean;
    hasProAccess: boolean;
    grantProAccess: () =>{t('common.promise')}<void>;
    ...
    closePaywall: () => void;
    ovens: Oven[];
    addOven: (newOvenData: Omit<Oven, 'id' | 'isDefault'>) =>{t('common.promise')}<Oven>;
    updateOven: (updatedOven: Oven) =>{t('common.promise')}<void>;
    deleteOven: (id: string) =>{t('common.promise')}<void>;
    setDefaultOven: (id: string) =>{t('common.promise')}<void>;
    preferredFlourId: string | null;
    setPreferredFlour: (id: string | null) => void;
    batches: Batch[];
    addBatch: (newBatch: Omit<Batch, 'id' | 'createdAt' | 'updatedAt'>) =>{t('common.promise')}<Batch>;
    ...
    updateLevain: (levain: Partial<Levain> & { id: string }) =>{t('common.promise')}<void>;
    deleteLevain: (id: string) =>{t('common.promise')}<void>;
    setDefaultLevain: (id: string) =>{t('common.promise')}<void>;
    addFeedingEvent: (levainId: string, event: Omit<FeedingEvent, 'id' | 'date'>) =>{t('common.promise')}<void>;
    importLevains: (levainsToImport: Levain[]) =>{t('common.promise')}<void>;
    goals: Goal[];
    addGoal: (goal: Omit<Goal, 'id' | 'createdAt' | 'updatedAt' | 'status' | 'progress'>) =>{t('common.promise')}<Goal>;
    updateGoal: (goal: Partial<Goal> & { id: string }) =>{t('common.promise')}<void>;
    deleteGoal: (id: string) =>{t('common.promise')}<void>;
    completeGoal: (id: string) =>{t('common.promise')}<void>;
    testSeries: TestSeries[];
    addTestSeries: (series: Omit<TestSeries, 'id' | 'createdAt' | 'updatedAt' | 'relatedBakes'>) =>{t('common.promise')}<TestSeries>;
    updateTestSeries: (series: Partial<TestSeries> & { id: string }) =>{t('common.promise')}<void>;
    ...
    addUserStyle: (style: Omit<DoughStyleDefinition, 'id' | 'createdAt'>) =>{t('common.promise')}<DoughStyleDefinition>;
    updateUserStyle: (style: DoughStyleDefinition) =>{t('common.promise')}<void>;
    deleteUserStyle: (id: string) =>{t('common.promise')}<void>;
    favorites: FavoriteItem[];
    toggleFavorite: (item: Omit<FavoriteItem, 'createdAt'>) =>{t('common.promise')}<void>;
