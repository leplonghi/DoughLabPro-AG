...
const CommunityBatchDetailPage: React.FC<CommunityBatchDetailPageProps> = ({ batchId, onLoadAndNavigate, onNavigate }) => {
    ...
    useEffect(() => {
        const fetchBatch = async () => {
            if (batchId) {
                const foundBatch = await getCommunityBatchById(batchId);
                setBatch(foundBatch);
            }
        };
        fetchBatch();
    ...
    if (!batch) {
        return (
            <div className="text-center p-8">
                <h2 className="text-xl font-bold text-slate-900 ">{t('community_detail.not_found')}</h2>
                <p className="mt-2 text-slate-600 ">{t('community_detail.not_found_desc')}</p>
                <button onClick={() => onNavigate('community')} className="mt-4 rounded-xl bg-lime-500 py-2.5 px-5 text-sm font-bold text-white shadow-lg shadow-lime-500/20 hover:bg-lime-600 transition-all">
        ...
        <div className="mx-auto max-w-4xl animate-fade-in">
            ...
            <div className="rounded-2xl bg-white  p-6 shadow-lg border border-slate-200  sm:p-10">
                ...
                <div className="mt-8 flex flex-col sm:flex-row items-center justify-between gap-4 border-t border-slate-200  pt-6">
                    <button onClick={() => onNavigate('community')} className="text-sm font-bold text-lime-600  hover:underline">
                        &larr; {t('community_detail.back_to_community')}
                    </button>
                    <button onClick={() => onLoadAndNavigate(baseConfig)} className="w-full sm:w-auto flex items-center justify-center gap-2 rounded-xl bg-lime-500 py-3 px-6 text-base font-bold text-white shadow-lg shadow-lime-500/20 transition-all hover:bg-lime-600 hover:scale-105 active:scale-95">
