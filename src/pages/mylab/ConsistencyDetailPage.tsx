                    ...
                    <div className="lg:col-span-2 space-y-6">
                        ...
                        <div className="rounded-2xl border border-slate-200  bg-white  p-6 shadow-sm">
                            ...
                            <div className="mt-6 pt-6 border-t border-slate-200  flex flex-col sm:flex-row items-center gap-3">
                                <select value={selectedBake} onChange={e => setSelectedBake(e.target.value)} className="w-full flex-grow rounded-xl border-slate-300  bg-white  text-sm py-2.5 px-3 text-slate-700  focus:ring-2 focus:ring-dlp-primary focus:border-transparent">
                                ...
                                <button onClick={handleAttachBake} disabled={!selectedBake} className="w-full sm:w-auto flex-shrink-0 flex items-center justify-center gap-2 rounded-xl bg-dlp-primary py-2.5 px-4 text-sm font-bold text-white shadow-lg shadow-dlp-primary/20 disabled:bg-slate-300 disabled:shadow-none transition-all hover:bg-dlp-primary hover:text-white-hover">
                        ...
                        <div className="rounded-2xl border border-slate-200  bg-white  p-6 shadow-sm">
                            ...
                            {isEditingNotes ? (
                                <div>
                                    <textarea rows={5} value={notes} onChange={e => setNotes(e.target.value)} className="w-full rounded-xl border-slate-300  bg-white  text-sm text-slate-700  p-3 focus:ring-2 focus:ring-dlp-primary focus:border-transparent" />
                                    <div className="flex justify-end gap-2 mt-3">
                                        ...
                                        <button onClick={handleSaveNotes} className="text-sm font-bold text-white bg-dlp-primary hover:bg-dlp-primary hover:text-white-hover px-4 py-2 rounded-lg">{t('common.save')}</button>
                                    </div>
                                </div>
                            ) : (
                                    ...
                                    <ul className="space-y-2">
                                        {series.parameters.steps.map((step, i) => (
                                            <li key={i} className="flex items-center gap-2 text-slate-700  bg-white  px-3 py-2 rounded-lg border border-slate-200 ">
                                                <span className="w-1.5 h-1.5 rounded-full bg-dlp-primary"></span>
                                                {step}
                                            </li>
                                        ))}
                                    </ul>
