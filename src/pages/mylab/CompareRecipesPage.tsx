...
const RecipeDetailColumn: React.FC<{ batch: Batch; onLoad: () => void; t: (key: string, options?: any) => string }> = ({ batch, onLoad, t }) => {
    ...
    return (
        <div className="rounded-xl border border-slate-200 bg-white  p-6 shadow-sm flex flex-col">
            ...
            <button
                onClick={onLoad}
                className="mt-6 w-full inline-flex items-center justify-center gap-2 rounded-lg bg-dlp-primary py-2.5 px-4 font-semibold text-white shadow-sm hover:bg-dlp-primary hover:text-white-hover"
            >
