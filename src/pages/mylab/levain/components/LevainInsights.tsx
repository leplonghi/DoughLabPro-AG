...
const HealthBar: React.FC<{ score: number }> = ({ score }) => {
    ...
    if (score > 80) { color = 'bg-dlp-primary'; text = t('ui.peak_health_346'); }
    else if (score > 50) { color = 'bg-dlp-primary'; text = t('ui.good_347'); }
        ...
        <div className="space-y-6">
            <div className="rounded-xl border border-slate-200 bg-white p-6 shadow-sm">
                <h3 className="text-lg font-bold text-slate-900 mb-6 flex items-center gap-2">
                    <SparklesIcon className="h-5 w-5 text-dlp-primary" />
                    Performance & Health
                </h3>
            ...
            <div className="rounded-xl border border-slate-200 bg-white p-6 shadow-sm">
                ...
                <div className="flex items-end gap-1 h-24">
                    {levain.feedingHistory.slice(0, 20).reverse().map((log, idx) => (
                        <div
                            key={idx}
                            className="flex-1 bg-lime-400 rounded-t-sm hover:bg-dlp-primary hover:text-white transition-colors relative group"
