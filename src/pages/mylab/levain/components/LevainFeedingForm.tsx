...
const LevainFeedingForm: React.FC<LevainFeedingFormProps> = ({ isOpen, onClose, levainId }) => {
  const { t } = useTranslation();
    ...
    return (
        <div
            className="fixed inset-0 z-40 flex items-center justify-center bg-lime-950/60 backdrop-blur-sm animate-[fadeIn_0.3s_ease-out]"
            onClick={onClose}
            role="dialog"
            aria-modal="true"
        >
            ...
            >
                <div className="flex items-center justify-between border-b border-slate-100 bg-slate-50/50 px-6 py-4">
                    <div className="flex items-center gap-3">
                        <div className="p-2 bg-lime-100 rounded-lg text-dlp-primary-hover">
                ...
                <form onSubmit={handleSubmit} className="p-6 space-y-6">
                    ...
                    <div className="bg-slate-50 p-4 rounded-xl border border-slate-200">
                        ...
                        <div className="flex items-center gap-4 mb-4">
                            ...
                            <div className="flex gap-2 items-end">
                                <button type="button" onClick={() => applyRatio(1, 1)} className="px-3 py-2 text-xs font-bold rounded-lg bg-white border border-slate-300 hover:border-dlp-primary hover:text-dlp-primary-hover transition-colors">1:1:1</button>
                                <button type="button" onClick={() => applyRatio(2, 2)} className="px-3 py-2 text-xs font-bold rounded-lg bg-white border border-slate-300 hover:border-dlp-primary hover:text-dlp-primary-hover transition-colors">1:2:2</button>
                                <button type="button" onClick={() => applyRatio(5, 5)} className="px-3 py-2 text-xs font-bold rounded-lg bg-white border border-slate-300 hover:border-dlp-primary hover:text-dlp-primary-hover transition-colors">1:5:5</button>
                            </div>
                        </div>
                    </div>
                    ...
                    <div className="grid grid-cols-2 gap-4 pt-2">
                        <div>
                            ...
                            <select
                                ...
                                className="w-full rounded-md border-slate-300 text-sm text-slate-700 focus:border-dlp-primary focus:ring-dlp-primary"
                            >
                        ...
                        <div>
                            ...
                            <input
                                ...
                                placeholder="24"
                                className="w-full rounded-md border-slate-300 text-sm focus:border-dlp-primary focus:ring-dlp-primary text-slate-700"
                            />
                        </div>
                    </div>
                    ...
                    <div className="flex justify-end gap-3 pt-4 border-t border-slate-100">
                        ...
                        <button type="submit" className="rounded-lg bg-gradient-to-br from-dlp-primary to-lime-700 px-6 py-2 text-sm font-bold text-white shadow-md shadow-lime-200 hover:from-dlp-primary-hover hover:to-lime-800 transition-all active:scale-95">{t('mylab.confirm_feeding')}</button>
                    </div>
                </form>
            </div>
        </div>
