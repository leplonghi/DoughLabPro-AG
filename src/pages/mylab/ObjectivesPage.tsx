...
function suggestGoalsFromHistory(userHistory: any[]): SuggestedGoal[] {
    ...
    if (userHistory.length > 2) {
        suggestions.push({
            title: t('common.mylab.increase_hydration_to_75'),
    ...
    suggestions.push({
        title: t('common.mylab.master_a_new_style_focaccia'),
        description: t('common.mylab.explore_the_complexities_of_highhydration_focaccia'),
        targetType: 'style',
        targetValue: 'FOCACCIA'
    });

    suggestions.push({
        title: t('common.mylab.bake_4_times_next_month'),
        description: t('common.mylab.maintain_consistency_in_your_practice_to_accelerat'),
        ...
        <div className="rounded-2xl bg-white  p-6 border border-slate-200  shadow-sm hover:shadow-md transition-all">
            <div className="flex justify-between items-start">
                ...
                <div className="flex items-center gap-2 flex-shrink-0 ml-4">
                    ...
                    {goal.status === 'ativo' && <button onClick={() => completeGoal(goal.id)} title={t('common.mylab.complete')} className="p-2 rounded-full hover:bg-green-50 transition-colors"><CheckCircleIcon className="h-5 w-5 text-dlp-primary hover:text-dlp-primary-hover" /></button>}
            ...
            {goal.status === 'ativo' && (
                <div className="mt-4">
                    <div className="flex justify-between items-center text-xs mb-2">
                        ...
                        <span className="font-bold text-dlp-primary-hover ">{goal.progress}%</span>
                    </div>
                    <div className="h-2.5 w-full rounded-full bg-slate-100  overflow-hidden">
                        <div className="h-full rounded-full bg-dlp-primary transition-all duration-500 ease-out" style={{ width: `${goal.progress}%` }} />
                    </div>
                </div>
            )}
        </div>
                ...
                <div className="animate-fade-in">
                    ...
                    >
                        <div className="mb-8 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
                            ...
                            <button onClick={() => handleOpenModal()} className="inline-flex w-full sm:w-auto items-center justify-center gap-2 rounded-xl bg-dlp-primary py-2.5 px-5 font-bold text-white shadow-lg shadow-dlp-primary/20 hover:bg-dlp-primary hover:text-white-hover transition-all hover:scale-105 active:scale-95">
                        ...
                        <div className="space-y-8">
                            ...
                            <div>
                                <h3 className="text-lg font-bold mb-4 flex items-center gap-2 text-slate-900 ">
                                    <SparklesIcon className="h-5 w-5 text-dlp-primary" />{t('common.mylab.ai_suggestions')}</h3>
                                <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                                    {suggestions.map(sugg => (
                                        <div key={sugg.title} className="rounded-2xl bg-slate-50  p-5 border border-slate-200  hover:border-lime-300 transition-colors">
                                            ...
                                            <button onClick={() => handleQuickAddSuggestion(sugg)} className="mt-4 text-xs font-bold text-dlp-primary-hover  hover:underline uppercase tracking-wider">{t('common.mylab.add_goal')}</button>
                                        </div>
                                    ))}
                                </div>
                            </div>
                            ...
                            <div>
                                <div className="border-b border-slate-200  mb-6">
                                    <nav className="-mb-px flex space-x-6" aria-label={t('common.mylab.tabs_2')}>
                                        <button onClick={() => setFilter('ativo')} className={`whitespace-nowrap py-3 px-1 border-b-2 font-bold text-sm transition-colors ${filter === 'ativo' ? 'border-dlp-primary text-dlp-primary-hover ' : 'border-transparent text-slate-500 hover:text-slate-700 '}`}>{t('common.mylab.active_2')}</button>
                                        <button onClick={() => setFilter('concluido')} className={`whitespace-nowrap py-3 px-1 border-b-2 font-bold text-sm transition-colors ${filter === 'concluido' ? 'border-dlp-primary text-dlp-primary-hover ' : 'border-transparent text-slate-500 hover:text-slate-700 '}`}>{t('common.mylab.completed')}</button>
                                    </nav>
                                </div>
