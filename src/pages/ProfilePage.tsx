...
const ProfilePage: React.FC<ProfilePageProps> = ({ onNavigate }) => {
  ...
  const renderInfoRow = (label: string, value: string | undefined) => (
    <div>
      ...
      <dd className="mt-1 text-slate-900  font-medium">{value || 'N/A'}</dd>
    </div>
  );
    ...
    <div>
      ...
      {type === 'select' ? (
        <select
          ...
          className="mt-1 block w-full rounded-xl border-slate-300  bg-white  text-slate-900  py-2 px-3 shadow-sm focus:border-lime-500 focus:outline-none focus:ring-lime-500 sm:text-sm"
        >
          {options?.map((opt) => (
        ...
        <input
          ...
          value={(formData[name] as any) || ''}
          onChange={handleInputChange}
          className="mt-1 block w-full rounded-xl border-slate-300  bg-white  text-slate-900  py-2 px-3 shadow-sm focus:border-lime-500 focus:outline-none focus:ring-lime-500 disabled:opacity-50 disabled:bg-slate-100 sm:text-sm"
          disabled={name === 'email'}
        />
      )}
    </div>
      ...
      <div className="mx-auto max-w-2xl animate-fade-in">
        ...
        <div className="rounded-2xl bg-white  p-6 shadow-lg border border-slate-200  sm:p-10">
            ...
            <div className="flex items-center justify-between">
              ...
              {!isEditing && (
                <button
                  ...
                  className="flex items-center gap-1.5 rounded-lg bg-slate-100  py-2 px-4 text-sm font-bold text-slate-700  hover:bg-slate-200 transition-colors"
                >
              ...
              {isEditing ? (
                <div className="space-y-6">
                  ...
                  <div className="flex items-center justify-end gap-4 border-t border-slate-200  pt-6">
                    <button
                      ...
                      className="rounded-xl py-2.5 px-5 text-sm font-bold text-slate-600  hover:bg-slate-100 transition-colors"
                    >
                      {t('profile.cancel')}
                    </button>
                    <button
                      onClick={handleSave}
                      className="rounded-xl bg-lime-500 py-2.5 px-5 text-sm font-bold text-white shadow-lg shadow-lime-500/20 hover:bg-lime-600 transition-all hover:scale-105 active:scale-95"
                    >
                ...
                <dl className="grid grid-cols-1 gap-x-4 gap-y-8 sm:grid-cols-2">
                  ...
                  <div>
                    ...
                    <dd className="mt-1">
                      <span
                        className={`inline-flex items-center gap-1.5 rounded-full px-2.5 py-1 text-xs font-semibold ${hasProAccess
          ...
          <div className="mt-10 border-t border-slate-200  pt-8">
            <div className="flex items-center justify-between">
              ...
              <button
                ...
                className="rounded-xl bg-lime-500 py-2 px-4 text-sm font-bold text-white shadow-lg shadow-lime-500/20 hover:bg-lime-600 transition-all hover:scale-105 active:scale-95"
              >
            ...
            <div className="mt-6 space-y-4">
              {ovens.length === 0 ? (
                ...
                ovens.map(oven => (
                  <div key={oven.id} className="flex items-center justify-between rounded-xl bg-slate-50  p-4 border border-slate-100 ">
                    <div>
                    ...
                    <div className="flex items-center gap-2">
                      <button onClick={() => setDefaultOven(oven.id)} title={t('profile.ovens.set_as_default')} className={`p-2 rounded-full transition-colors ${oven.isDefault ? 'text-yellow-400' : 'text-slate-400  hover:text-yellow-400'}`}>
                        {oven.isDefault ? <SolidStarIcon className="h-5 w-5" /> : <StarIcon className="h-5 w-5" />}
                      </button>
                      <button onClick={() => { setEditingOven(oven); setIsOvenModalOpen(true); }} className="p-2 rounded-full text-slate-500  hover:bg-slate-200 transition-colors">
                        <PencilIcon className="h-5 w-5" />
                      </button>
                      <button onClick={() => handleDeleteOven(oven.id)} className="p-2 rounded-full text-red-500 hover:bg-red-100 transition-colors">
          ...
          <div className="mt-10 border-t border-slate-200  pt-8">
            ...
            <div className="mt-6 space-y-4">
              {levains.length === 0 ? (
                ...
                levains.map(levain => (
                  <div key={levain.id} className="flex items-center justify-between rounded-xl bg-slate-50  p-4 border border-slate-100 ">
                    <div>
                    ...
                    <div className="flex items-center gap-1 sm:gap-2">
                      <button onClick={() => onNavigate('mylab/levain')} title={t('profile.levains.manage')} className="p-2 rounded-lg text-sm font-bold text-lime-600  hover:bg-lime-100 transition-colors">
                        {t('profile.levains.manage')}
                      </button>
                      <button onClick={() => setDefaultLevain(levain.id)} title={t('profile.levains.set_as_default')} className={`p-2 rounded-full transition-colors ${levain.isDefault ? 'text-yellow-400' : 'text-slate-400  hover:text-yellow-400'}`}>
                        {levain.isDefault ? <SolidStarIcon className="h-5 w-5" /> : <StarIcon className="h-5 w-5" />}
                      </button>
                      <button onClick={() => { setEditingLevain(levain); setIsLevainModalOpen(true); }} className="p-2 rounded-full text-slate-500  hover:bg-slate-200 transition-colors">
                        <PencilIcon className="h-5 w-5" />
                      </button>
                      <button onClick={() => handleDeleteLevain(levain.id)} className="p-2 rounded-full text-red-500 hover:bg-red-100 transition-colors">
                        <TrashIcon className="h-5 w-5" />
                      </button>
                    </div>
                  </div>
          ...
          <div className="mt-10 border-t border-slate-200  pt-8">
            ...
            <div className="mt-4 grid grid-cols-1 gap-4 sm:grid-cols-2">
              <button
                ...
                className="flex items-center gap-3 rounded-xl bg-slate-50  p-4 text-left transition hover:bg-slate-100 border border-slate-100 "
              >
          ...
          <div className="mt-10 border-t border-slate-200  pt-8">
            ...
            <div className="mt-4 grid grid-cols-1 gap-3 sm:grid-cols-2">
              <button
                ...
                className="flex items-center justify-between rounded-xl bg-slate-50  p-3 text-left transition hover:bg-slate-100 border border-slate-100 "
              >
                <div className="flex items-center gap-2">
          ...
          <div className="mt-10 border-t border-slate-200  pt-8">
            <button
              onClick={logout}
              className="inline-flex items-center justify-center gap-2 rounded-xl bg-slate-200  py-3 px-6 text-base font-bold text-slate-700  shadow-sm transition-all hover:bg-slate-300 sm:w-auto"
            >
