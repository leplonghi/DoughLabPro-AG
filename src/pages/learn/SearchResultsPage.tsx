            ...
            <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 mt-8">
                {results.length === 0 ? (
                    <div className="text-center py-12 bg-white rounded-2xl border border-stone-200 border-dashed">
                        ...
                        <button onClick={() => navigate('learn')} className="mt-6 px-6 py-2 bg-dlp-primary-hover hover:bg-dlp-primary hover:text-white text-white rounded-lg font-bold transition-colors">{t('learn.browse_library')}</button>
                    </div>
                ) : (
                    <div className="space-y-4">
                        {results.map(({ article, matches }) => (
                            <div
                                ...
                                className="bg-white p-6 rounded-xl border border-stone-200 hover:border-dlp-primary hover:shadow-md transition-all cursor-pointer group"
                            >
                                <div className="flex justify-between items-start">
                                    <div>
                                        <div className="flex items-center gap-2 mb-2">
                                            <span className="text-xs font-bold text-dlp-primary-hover uppercase tracking-wider">
                                                {article.category}
                                            </span>
                                            {matches.length > 0 && (
                                        ...
                                        <h3 className="text-xl font-bold text-slate-900 mb-2 group-hover:text-dlp-primary-hover transition-colors">
                                            {article.title}
                                        </h3>
                                    ...
                                    <ArrowRightIcon className="w-5 h-5 text-slate-400 group-hover:text-dlp-primary transition-colors mt-2" />
                                </div>
                            </div>
                        ))}
                    </div>
