...
export const LearnCollapseSection: React.FC<{
    ...
    return (
        <div className="border-b border-slate-100 last:border-0">
            ...
            >
                <h3 className="text-xl font-bold text-slate-900 group-hover:text-dlp-primary-hover transition-colors flex items-center gap-3">
                    {icon && <span className="text-dlp-primary">{icon}</span>}
                    {title}
                </h3>
                <div className={`p-2 rounded-full transition-all duration-300 ${isExpanded ? 'bg-lime-100 text-dlp-primary-hover rotate-180' : 'bg-slate-50 text-slate-400 group-hover:bg-lime-50 group-hover:text-dlp-primary'}`}>
