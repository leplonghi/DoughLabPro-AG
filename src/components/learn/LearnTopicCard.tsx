...
const LearnTopicCard: React.FC<LearnTopicCardProps> = ({ icon, title, description, onClick, isPro }) => {
    ...
    return (
        ...
        >
            {isPro && (
                <div className="absolute top-0 right-0 bg-dlp-brand text-white text-[10px] font-bold px-2 py-1 rounded-bl-lg shadow-sm z-10 uppercase tracking-wide">{t('learn.pro')}</div>
            )}
            ...
            <p className="mt-4 text-sm font-semibold text-slate-600 group-hover:text-dlp-brand transition-colors">
