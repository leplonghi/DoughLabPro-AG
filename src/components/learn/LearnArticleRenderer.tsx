    ...
    <div className="bg-white rounded-2xl border border-stone-200 overflow-hidden shadow-sm mb-6">
        <div className="px-6 sm:px-8 py-6 border-b border-stone-100 bg-stone-50/30">
            <h3 className="text-xl font-bold text-slate-900 flex items-center gap-3">
                {icon && <span className="text-dlp-primary">{icon}</span>}
                {title}
            </h3>
        </div>
                        ...
                        <div className="flex flex-wrap justify-center gap-2 py-3">
                            <button
                                ...
                                className={`flex min-h-11 w-full items-center justify-center gap-2 rounded-full border-2 px-6 py-3 text-sm font-bold transition-all sm:w-auto ${localMode === 'technical'
                                    ? 'bg-dlp-primary-hover border-dlp-primary-hover text-white shadow-lg shadow-lime-900/20 scale-105'
                                    : 'bg-white border-stone-200 text-stone-500 hover:border-dlp-primary hover:text-dlp-primary-hover'
                                    }`}
                            >
                ...
                {localMode === 'grandma' && (
                    finalData.grandmaVersion ? (
                        ...
                        <div className="p-8 bg-white rounded-2xl text-center text-slate-500 border border-stone-200">
                            ...
                            <button onClick={() => handleModeChange('technical')} className="block mx-auto mt-4 text-dlp-primary-hover hover:underline">{t('learn.return_main', { defaultValue: 'Return to Main Article' })}</button>
                        </div>
                    )
                )}
                            ...
                            <ul className="space-y-2 mb-6">
                                {summaryData.keyPoints.map((point, i) => (
                                    <li key={i} className="flex items-start gap-2 text-slate-600">
                                        <CheckIcon className="w-5 h-5 text-dlp-primary-hover flex-shrink-0 mt-0.5" />
                                        <span>{point}</span>
                                    </li>
                                ))}
                            </ul>
                                ...
                                <ul className="space-y-3">
                                    {finalData.technicalFoundations.map((item, i) => (
                                        <li key={i} className="flex items-start gap-3 text-slate-700">
                                            <div className="w-1.5 h-1.5 rounded-full bg-dlp-primary mt-2 flex-shrink-0" />
                                            <span>{item}</span>
                                        </li>
                                    ))}
                                </ul>
                                    ...
                                    {finalData.faq.map((item, i) => (
                                        <div key={i} className="bg-stone-50 p-4 rounded-xl border border-stone-200">
                                            <h4 className="font-bold text-slate-900 mb-2 flex items-start gap-2">
                                                <QuestionMarkCircleIcon className="h-5 w-5 text-dlp-primary-hover flex-shrink-0 mt-0.5" />
                                                {item.q}
                                            </h4>
