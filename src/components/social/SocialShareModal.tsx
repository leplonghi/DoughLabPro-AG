...
const SocialShareModal: React.FC<SocialShareModalProps> = ({ isOpen, onClose, config, results }) => {
    ...
    // DoughConfig does not have fermentationTime — derive a display label from available data
    const fermentationLabel = (config as any).fermentationTime ?? '—';
    const styleName = config.recipeStyle.charAt(0).toUpperCase() + config.recipeStyle.slice(1);

    return (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-white/20 p-4 backdrop-blur-sm animate-fade-in">
            <div className="relative w-full max-w-md rounded-2xl bg-dlp-bg-card p-6 shadow-dlp-lg border border-dlp-border">
                ...
                <h3 className="mb-6 text-xl font-bold text-dlp-text-primary flex items-center gap-2">
                    <ShareIcon className="h-5 w-5 text-dlp-accent" />{t('common.share_your_formula')}
                </h3>
                    ...
                    >
                        ...
                        <div className="relative z-10 flex flex-col items-center justify-center my-4">
                            <div className="relative w-36 h-36">
                                ...
                                <div className="absolute inset-0 flex flex-col items-center justify-center text-center">
                                    ...
                                    <span className="text-[8px] font-bold text-lime-300 uppercase tracking-widest">{t('calculator:form.hydration')}</span>
                                </div>
                            </div>
                        </div>
                        ...
                        <div className="relative z-10 grid grid-cols-2 gap-3 mb-4 bg-white/5 rounded-xl p-3 border border-white/5 backdrop-blur-sm">
                            <div className="text-center p-1 border-r border-white/10">
                                ...
                                <span className="block text-lg font-bold text-white">{fermentationLabel}</span>
                            </div>
                        ...
                        <div className="relative z-10 mt-auto pt-3 flex justify-between items-center border-t border-white/10">
                            <div>
                                ...
                                <p className="font-bold text-xs text-white tracking-wider uppercase">{t('common:doughlab_pro')}</p>
                            </div>
                            <div className="bg-white/10 p-1.5 rounded-lg backdrop-blur-md">
                                <img src="/app-logo.png" alt={t('learn:doughlab')} className="h-6 w-auto invert brightness-0" />
                            </div>
                        </div>
                    </div>
                </div>
                ...
                <p className="mt-4 text-center text-xs text-dlp-text-muted">{t('common:perfect_for_instagram_stories_or_sharing_with_friends')}</p>
            </div>
        </div>
    );
};
