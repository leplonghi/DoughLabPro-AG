        ...
        <div className="bg-white rounded-xl shadow-sm border border-stone-200 overflow-hidden">
            <div className="bg-stone-50 px-4 py-3 border-b border-stone-200 flex items-center justify-between">
                <h3 className="font-semibold text-stone-900 flex items-center">
                    <Gauge className="w-5 h-5 mr-2 text-dlp-primary-hover" />{t('common.technical_profile')}</h3>
                <span className={`px-2 py-1 rounded text-xs font-medium 
          ${profile.difficulty === t('ui.expert_372') ? 'bg-red-100 text-red-800' :
                        profile.difficulty === t('ui.hard_373') ? 'bg-orange-100 text-orange-800' :
                            profile.difficulty === t('ui.medium_374') ? 'bg-yellow-100 text-yellow-800' :
