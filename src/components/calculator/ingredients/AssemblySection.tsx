...
export const AssemblySection: React.FC<AssemblySectionProps> = ({ style, selectedIncrements, onUpdateIncrements, bakingTempC, bakeType = 'PIZZAS' }) => {
    ...
    return (
        <AccordionSection
            ...
            description={bakeType === 'PIZZAS' ? t('calculator.assembly_toppings_desc_pizza') : t('calculator.assembly_toppings_desc_bread')}
            icon={<PlusIcon />}
        >
            <div className="space-y-4">
                {/* Header - Already handled by AccordionSection partially, but keeping specific status badge */}
                            ...
                            {selectedIncrements.map(item => {
                                ...
                                return (
                                    <div key={item.id} className="relative group">
                                        <div className="flex items-center gap-2 p-2 bg-slate-50 rounded-lg border border-slate-100 group-hover:border-[#51a145] transition text-left">
                                            ...
                                            >
                                                <div className="flex items-center gap-1">
                                                    <input
                                                        ...
                                                        className="w-16 px-2 py-1 text-xs border border-slate-200 rounded focus:border-dlp-brand focus:ring-1 focus:ring-dlp-brand outline-none"
                                                        min="0"
                                                        step="10"
                                                    />
                                            ...
                                            <div className="flex items-center gap-1">
                                                {flavorMatch && (
                                                    <button
                                                        onClick={(e) => { e.stopPropagation(); setSelectedFlavorComponent(flavorMatch); }}
                                                ...
                                                <button onClick={(e) => { e.stopPropagation(); handleRemove(item.id); }} className="p-1 text-slate-300 hover:text-red-500 transition">
                ...
                {/* Create Custom - Prominent Button */}
