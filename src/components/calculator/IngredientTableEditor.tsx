    ...
    return (
        <div className="w-full bg-dlp-bg-card border border-dlp-border rounded-xl overflow-hidden shadow-sm">
            <div className="p-4 bg-dlp-bg-muted/30 border-b border-dlp-border flex justify-between items-center">
                <h4 className="font-bold text-sm text-dlp-text-primary uppercase tracking-wide">Custom Ingredients</h4>
                <button
                    onClick={handleAdd}
                    disabled={isAdding}
            ...
            <div className="divide-y divide-dlp-border">
                ...
                <div className="grid grid-cols-12 gap-2 p-3 bg-dlp-bg-muted/50 text-[10px] uppercase font-bold text-dlp-text-muted select-none">
                    <div className="col-span-5 pl-2">Ingredient</div>
                            ...
                            {editingId === ing.id ? (
                                ...
                                <>
                                    ...
                                    <div className="col-span-1 flex justify-end gap-1 opacity-10 group-hover:opacity-100 transition-opacity">
                                        <button onClick={() => handleEdit(ing)} className="p-1.5 text-dlp-text-secondary hover:text-dlp-accent hover:bg-dlp-bg-muted rounded transition-colors" title="Edit">
                                            <Edit2 className="w-3.5 h-3.5" />
                                        </button>
                                        <button onClick={() => handleDelete(ing.id)} className="p-1.5 text-dlp-text-secondary hover:text-dlp-error hover:bg-red-50 rounded transition-colors" title="Remove">
                ...
                {isAdding && (
                    <div className="grid grid-cols-12 gap-2 p-3 items-start bg-dlp-accent/5 animate-fade-in-down border-t-2 border-dlp-accent/20">
                        ...
                        <div className="col-span-1 flex flex-col justify-start gap-1">
                            <button onClick={handleSave} className="p-1.5 bg-dlp-success text-white rounded hover:bg-dlp-accent-hover transition-colors shadow-sm" title="Add"><Plus className="w-4 h-4" /></button>
                            <button onClick={cleanup} className="p-1.5 text-dlp-error hover:bg-dlp-error/10 rounded transition-colors" title="Cancel"><X className="w-4 h-4" /></button>
                        </div>
                    </div>
                )}
            </div>
