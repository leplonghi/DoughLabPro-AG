...
const AuthModal: React.FC<AuthModalProps> = ({ isOpen, onClose }) => {
  ...
  const handleEmailLogin = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!email || !password) {
      setError(t("ui:auth_messages.fill_all_fields"));
      return;
    }
    setIsLoading(true);
    setError(null);
  ...
  const handleSignup = async (e: React.FormEvent) => {
    ...
    if (password.length < 6) {
      setError(t("ui:auth_messages.password_min_6"));
      return;
    }
    setIsLoading(true);
    setError(null);
    try {
      ...
      if (err.code === "auth/email-already-in-use") {
        setError(t("ui:auth_messages.email_in_use"));
      } else {
        setError(t("ui:auth_messages.failed_create_account"));
      }
    } finally {
      setIsLoading(false);
    }
  ...
  const handleResetPassword = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!email) {
      setError(t("ui:auth_messages.enter_email"));
      return;
    }
    setIsLoading(true);
    setError(null);
    try {
      ...
      if (err.code === "auth/user-not-found") {
        setError(t("ui:auth_messages.no_account_found"));
      } else {
        setError(t("ui:auth_messages.failed_reset"));
      }
    } finally {
      setIsLoading(false);
    }
  ...
  const handleGuestLogin = async () => {
    setIsLoading(true);
    try {
      // For guest login, simply close the modal to allow offline use
      onClose();
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-white/20 backdrop-blur-sm animate-fade-in">
      <div
      ...
      >
        ...
        <div className="h-32 bg-gradient-to-br from-dlp-accent to-dlp-accent-hover flex items-center justify-center relative overflow-hidden">
          ...
          <div className="text-center z-10 p-4">
            <h2 className="text-3xl font-bold text-white drop-shadow-md">
              {view === "forgot-password"
                ? t("ui.reset_password_357")
                : t("ui.doughlab_pro_358")}
            </h2>
          ...
          <button
            onClick={onClose}
            className="absolute top-4 right-4 p-2 bg-black/20 hover:bg-white/20 text-white rounded-full transition-colors"
          >
        ...
        <div className="p-6 pt-8">
          ...
          {view === "login" && (
            <form onSubmit={handleEmailLogin} className="space-y-4">
              <div>
                <label className="block text-sm font-medium text-dlp-text-secondary mb-1">
                  {t("general.email")}
                </label>
            ...
            <form onSubmit={handleSignup} className="space-y-4">
              ...
              <div>
                ...
                <input
                  ...
                  onChange={(e) => setPassword(e.target.value)}
                  className="block w-full px-3 py-2.5 border border-dlp-border rounded-xl focus:ring-2 focus:ring-dlp-accent focus:border-dlp-accent transition-colors bg-dlp-bg-card text-dlp-text-primary placeholder-dlp-text-muted"
                  placeholder={t("ui:auth_messages.create_password", {
                    defaultValue: "Create a password (min 6 chars)",
                  })}
                  required
                  minLength={6}
                />
            ...
            <div className="space-y-4">
              {resetSent ? (
                <div className="text-center py-6">
                  ...
                  <h3 className="text-lg font-semibold text-dlp-text-primary">
          ...
          {view !== "forgot-password" && (
            <div className="relative my-6">
              ...
              <div className="relative flex justify-center text-sm">
                <span className="px-2 bg-dlp-bg-card text-dlp-text-muted">
