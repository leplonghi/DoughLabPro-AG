            ...
            <div className="w-full max-w-2xl max-h-[90vh] flex flex-col rounded-2xl bg-dlp-bg-card shadow-dlp-lg ring-1 ring-dlp-border text-dlp-text-primary overflow-hidden">
                        ...
                        <div className="flex items-center gap-3 mb-1">
                            ...
                            {component.isStandard && (
                                <span className="px-2 py-0.5 rounded-full bg-dlp-brand/10 text-dlp-brand-hover text-xs font-bold uppercase tracking-wide flex items-center gap-1">
                                    <CheckCircleIcon className="w-3 h-3" />{t('styles:standard_23')}</span>
                            )}
                        </div>
                ...
                <div className="overflow-y-auto p-6 space-y-8 custom-scrollbar">
                    ...
                    <div className="bg-dlp-bg-muted rounded-xl p-5 border border-dlp-border">
                        <h3 className="text-sm font-bold text-dlp-text-secondary uppercase tracking-widest mb-4">{t('ui:flavor_texture_profile')}</h3>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div className="space-y-3">
                            <h3 className="flex items-center gap-2 text-sm font-bold text-dlp-text-secondary uppercase tracking-widest border-b border-dlp-border pb-2">
                                <FireIcon className="w-4 h-4" />{t('learn:thermal_behavior')}</h3>
                        ...
                        <div className="md:col-span-1 space-y-6">
                            <div>
                                <h4 className="flex items-center gap-2 text-xs font-bold text-dlp-text-muted uppercase mb-2">
                                    <GlobeAltIcon className="w-4 h-4" />{t('common:origin')}</h4>
                                <p className="text-sm font-medium">{component.origin}</p>
                            </div>
                            <div>
                                <h4 className="text-xs font-bold text-dlp-text-muted uppercase mb-2">{t('styles:styles.montreal_bagel_qa_tradition_ctx')}</h4>
                                <p className="text-xs text-dlp-text-secondary leading-relaxed">{component.historyContext}</p>
                            </div>
                            <div>
                                <h4 className="text-xs font-bold text-dlp-text-muted uppercase mb-2">{t('ui:best_for')}</h4>
                        ...
                        <div className="md:col-span-2 space-y-6">
                            ...
                            <div className="bg-yellow-50 border border-yellow-200 rounded-lg p-4">
                                <h4 className="flex items-center gap-2 text-sm font-bold text-yellow-800 uppercase mb-2">
                                    <AlertTriangleIcon className="w-4 h-4" />{t('learn:technical_notes')}</h4>
                            ...
                            <div>
                                <h4 className="text-sm font-bold text-dlp-text-secondary uppercase mb-3">{t('learn:classic_combinations')}</h4>
                    ...
                    <div className="border-t border-dlp-border pt-6">
                        <h4 className="text-sm font-bold text-dlp-text-secondary uppercase mb-4">{t('ui:references_authority')}</h4>
